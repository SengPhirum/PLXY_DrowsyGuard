...
# ---------- the reflex guard ----------
#
# All that survives of the old reflex detector, and deliberately so: a closure that
# began with the mouth flung wide open has to outlast REFLEX_MAX_S before it counts
# as a microsleep. The feature is gone; this guard is not, because without it an
# involuntary reflex becomes a "microsleep" announcement aimed at a driver who is
# wide awake.

def test_a_mouth_open_closure_inside_the_reflex_window_is_not_a_microsleep():
    an = BehaviorAnalyzer(fps=FPS)
    feed(an, 90)
    states = feed(an, 30, p_closed=0.95, jaw=1.32)     # 1.0 s, inside REFLEX_MAX_S
    states += feed(an, 10, jaw=1.32)                   # eyes reopen
    assert 'microsleep' not in events_of(states)


def test_slow_closure_without_mouth_movement_is_still_a_microsleep():
    """The guard must be specific to a mouth flung wide, not blanket."""
    an = BehaviorAnalyzer(fps=FPS)
    feed(an, 90)
...
def test_a_mouth_open_closure_that_outlasts_the_window_is_a_microsleep():
    """The other side of the guard, and why it is a delay rather than a veto.

    A yawn opens the mouth wide and closes the eyes, so it clears REFLEX_JAW_DELTA
    comfortably - the absolute level cannot tell the two apart. What proves it was
    not a reflex is that the closure is still going after REFLEX_MAX_S. Turning the
    delay into a veto would silence a genuine drowsiness cue.
    """
    an = BehaviorAnalyzer(fps=FPS)
    feed(an, 90)
    states = feed(an, 30, jaw=1.32)                    # 1 s of open mouth
    states += feed(an, 45, p_closed=0.95, jaw=1.32)    # then the eyes close, 1.5 s
    states += feed(an, 5, jaw=1.05)
    assert 'microsleep' in events_of(states), 'the closure must still be alarmed on'
...
def test_reset_clears_state():
    ...
    an.reset()
    s = feed(an, 1)[-1]
    assert s.yawn_rate == 0 and not s.baselines_ready
